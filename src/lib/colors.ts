...
const STOPS: Array<{ frp: number; color: RGB }> = [
  { frp: 0, color: [127, 29, 29] }, // #7f1d1d deep red
...
export function frpColor(frp: number, alpha = 210): [number, number, number, number] {
  if (frp <= STOPS[0].frp) return [...STOPS[0].color, alpha]
...
/** Point radius in px by FRP — sublinear so megafires don't swallow the map. */
export function frpRadiusPx(frp: number): number {
  return 1.3 + Math.min(5, Math.sqrt(Math.max(frp, 0)) * 0.45)
}
