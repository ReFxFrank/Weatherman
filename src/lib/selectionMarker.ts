...
/**
 * Amber ring highlighting the selected fire detection — a maplibre-native
 * circle layer (globe-safe), re-synced after style swaps like the other
 * native layers.
 */
const SOURCE = 'ember-selection'
const LAYER = 'ember-selection-ring'

export function syncSelectionMarker(map: MapLibreMap, point: { lon: number; lat: number } | null) {
  try {
