...
export function FilterPanel() {
  ...
  if (!s.panelOpen) {
    return (
      <button
        ...
        title="Open filters"
        className={`absolute left-4 top-28 z-10 flex h-10 w-10 items-center justify-center text-slate-300 transition-colors hover:text-amber-300 ${glass}`}
      >
  ...
  return (
    <aside
      className={`absolute left-4 top-28 z-10 flex max-h-[calc(100%-8.5rem)] w-60 flex-col ${glass}`}
    >
